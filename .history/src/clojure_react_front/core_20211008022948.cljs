...
[:tbody
  ...
  [:th {:on-click #(let [old-data @data] (reset! data (sort (if(@current-temp-asc) [by-current-temp-asc] [by-current-temp-desc]) old-data)) (reset! current-temp-asc @current-temp-asc) )} "current temperature"]
