...
(go (let [response (<! (http/get "https://raw.githubusercontent.com/icyrockcom/country-capitals/master/data/country-list.csv" {:with-credentials? false :type "text/csv; charset=utf-8"}))]
  (prn (:status response))
  (prn (map ({fn [item] (first item) (second item) (.floor js/Math(* 100 (.random js/Math)))   (.floor js/Math(* 100 (.random js/Math))) (.floor js/Math(* 100 (.random js/Math))) (.floor js/Math(* 100 (.random js/Math))) (.floor js/Math(* 100 (.random js/Math)))}) (csv/parse (:body response))))))
