...
;; Atoms
(def data (r/atom []))
(def current-temp-asc (r/atom false))
...
[:tbody
  ...
  [:th {:on-click #(let [old-data @data] (reset! data (sort (if(@current-temp-asc) by-current-temp-asc by-current-temp-desc) old-data)) (reset! current-temp-asc not(current-temp-asc)) (prn (@current-temp-asc)))} "current temperature"]
...
(go (let [response (<! (http/get "https://raw.githubusercontent.com/icyrockcom/country-capitals/master/data/country-list.csv" {:with-credentials? false :type "text/csv; charset=utf-8"}))]
  (reset! data (seq (map (fn [item] {:country (first item) :capital  (second item) :current-temp (.floor js/Math(* 100 (.random js/Math))) :temp-in-12-hrs (.floor js/Math(* 100 (.random js/Math)))  :temp-in-24-hrs (.floor js/Math(* 100 (.random js/Math))) :temp-in-3-days (.floor js/Math(* 100 (.random js/Math))) :temp-in-7-days (.floor js/Math(* 100 (.random js/Math)))} ) (csv/parse (:body response)))))))
