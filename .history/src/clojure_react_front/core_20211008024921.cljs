...
[:tbody
  ...
  [:th "Country"]
  [:th "Capital"]
  [:th {:on-click #(let [old-data @data old-temp @current-temp] (reset! current-temp (if (= old-temp true) false true)) (reset! data (sort by-current-temp-asc old-data)) )} "current temperature"]
