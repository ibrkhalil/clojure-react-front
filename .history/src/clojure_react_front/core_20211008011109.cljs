...
[:tbody
  ...
  ;; Map random temperatures to the table and drop the first record cause it's already filled in the table's heading
   (map (fn [item] [:tr {:key item} [:td (:country item)] [:td (:item capital)] [:td " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"]]) @data)
