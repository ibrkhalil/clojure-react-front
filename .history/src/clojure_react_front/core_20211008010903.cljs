...
[:tbody
  ...
  ;; Map random temperatures to the table and drop the first record cause it's already filled in the table's heading
   (map (fn [item] [:tr {:key item} [:td (first item)] [:td (second item)] [:td (third item) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"]]) @data)
