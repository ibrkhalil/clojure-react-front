...
[:tbody
  [:tr
  [:th "Country"]
  [:th "Capital" {:on-click #(let [old-data @data old-capital @current-capital] (reset! current-capital (if (= old-capital true) false true)) (reset! data (sort by-current-capital old-data)) )}]
