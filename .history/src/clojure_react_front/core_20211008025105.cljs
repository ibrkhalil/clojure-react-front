...
[:tbody
  ...
  [:th "Country"]
  [:th "Capital"]
  [:th {:on-click #(let [old-data @data old-temp @current-temp] (prn @current-temp) (reset! current-temp (if (= old-temp true) false true)) (reset! data (sort by-current-temp old-data)) )} "current temperature"]
