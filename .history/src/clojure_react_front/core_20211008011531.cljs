...
[:tbody
  ...
  ;; Map random temperatures to the table and drop the first record cause it's already filled in the table's heading
   (map (fn [item] [:tr {:key (:country item)} [:td (:country item)] [:td (:capital item)] [:td " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"] [:td (.floor js/Math(* 100 (.random js/Math))) " Celsius"]]) @data)
