...
[:tbody
  ...
  [:th "Country"]
  [:th "Capital"]
  [:th {:on-click #(let [old-data @data] (reset! current-temp if(= (deref current-temp true)) false true) (reset! data (sort by-current-temp-asc old-data)) )} "current temperature"]
